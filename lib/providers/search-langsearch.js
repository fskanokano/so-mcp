 ...
 * 职责：把 UnifiedSearch 输入映射为 LangSearch 请求体（query/count/freshness/contents/includeDomains/excludeDomains），把 data.webPages.value 结果归一为 UnifiedSearch 输出。
 ...
 * @property {number} [maxResults] 来源数量上限（钳制 1..50）
...
/**
 * 解析新鲜度窗口：统一 freshness 与 timeRange/time_range 互认并归一为上游形态。
 * 别名 day/week/month/year（含单字母）折为 oneDay/oneWeek/oneMonth/oneYear；
 * 非法取值直接丢弃，避免上游 400。
 * @param {any} params 统一搜索输入
...
function resolveFreshness(params) {
  ...
  const text = native.trim();
  const lower = text.toLowerCase();
  if (lower === 'nolimit' || lower === 'no_limit' || lower === 'all' || lower === 'none') return 'noLimit';
  if (lower === 'oneday' || lower === 'day' || lower === 'd' || lower === 'daily' || lower === '24h') return 'oneDay';
  if (lower === 'oneweek' || lower === 'week' || lower === 'w' || lower === 'weekly') return 'oneWeek';
  if (lower === 'onemonth' || lower === 'month' || lower === 'm' || lower === 'monthly') return 'oneMonth';
  if (lower === 'oneyear' || lower === 'year' || lower === 'y' || lower === 'yearly') return 'oneYear';
  if (/^\d{4}-\d{2}-\d{2}(\.\.\d{4}-\d{2}-\d{2})?$/.test(text)) return text;
  return undefined;
}

/**
 * 解析日期范围：原生 date_range 优先；统一 fromDate/toDate 双界齐备时拼 YYYY-MM-DD..YYYY-MM-DD。
 * 单界无法表达上游区间语义，直接丢弃，避免上游 400。
 * @param {any} params 统一搜索输入（含原生 date_range/dateRange）
 * @returns {string|undefined} 可填入上游 freshness 的日期形态，条件不足时回 undefined
 */
...
/**
 * 归一域名列表为上游原生数组：数组或逗号串皆可，去协议头与路径，只留裸域。
 * @param {unknown} value 域名白/黑名单输入
 * @returns {string[]|undefined} 裸域数组，无过滤需求时回 undefined
 */
function pickDomainList(value) {
  const list = Array.isArray(value)
    ? value.map(cleanDomain).filter(Boolean)
    : typeof value === 'string' && value.trim()
      ? value.split(',').map(cleanDomain).filter(Boolean)
      : [];
  return list.length > 0 ? list : undefined;
}

/**
 * 从上游响应收集可映射条目：上游标准形态 data.data.webPages.value 优先，
 * results/data 包裹与 web/items 形态兼容补量。
...
function collectItems(data) {
  ...
  // 上游标准形态（英文查询主形态）：data 层内的 webPages.value。
  if (data && data.data && data.data.webPages && Array.isArray(data.data.webPages.value)) {
    return data.data.webPages.value;
  }
  if (data && data.webPages && Array.isArray(data.webPages.value)) return data.webPages.value;
...
/**
 * 从单条结果拼装正文：highlights 数组拼接优先，其次 text/content/snippet/summary。
 * 上游全文模式回 text 替代 snippet，text 优先保证正文回量。
...
/**
 * LangSearch 搜索：POST /v1/web-search，请求体含 query/count/freshness/contents/域名数组。
...
export async function langsearchSearch(params, deps) {
  ...
  // 日期：原生 freshness 优先；缺失时 fromDate/toDate 双界齐备折 freshness 区间。
  const freshness = resolveFreshness(params) ?? resolveDateRange(params);
  ...
  // 全文开关：summary 语义折为 contents.text（true 取全文，false 取 snippet）。
  if (resolveSummary(params)) body.contents = { text: true };
  if (freshness) body.freshness = freshness;
  // 域名过滤走上游原生数组，不再折 site 语法拼 query。
  const includeList = pickDomainList(params && (params.includeDomains ?? anyParams.include_domains));
  const excludeList = pickDomainList(params && (params.excludeDomains ?? anyParams.exclude_domains));
  if (includeList) body.includeDomains = includeList;
  if (excludeList) body.excludeDomains = excludeList;
  ...
  if (res.status === 402 || res.status === 403) {
    // 402 多为余额不足，403 多为无权限；读正文嗅探后拆分，两者均不可重试。
    let bodyText = '';
    try {
    ...
    const looksQuota =
      ...
      lower.includes('402');
    const asCredential = res.status === 403 ? !looksQuota || looksAuth : looksAuth && !looksQuota;
    throw fail(
      asCredential ? 'CREDENTIAL_MISSING' : 'INSUFFICIENT_CREDIT',
      asCredential ? 'LangSearch 密钥无效或无权限' : 'LangSearch 余额不足',
