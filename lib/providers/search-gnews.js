...
/** 缺省语言（上游 lang，不传即不限，旧默认 en 会丢中文结果）。 */
const DEFAULT_LANG = '';
/** 缺省国家（上游 country，不传即不限，旧默认 us 会丢非美结果）。 */
...
export async function gnewsSearch(params, deps) {
  ...
  if (res.status === 403) {
    // 403 双因：密钥无权限 vs 额度耗尽，均不可重试；读正文嗅探后拆分。
    let bodyText = '';
    try {
    ...
    throw fail(
      looksAuth ? 'CREDENTIAL_MISSING' : 'INSUFFICIENT_CREDIT',
      looksAuth ? 'GNews 密钥无效或无权限' : 'GNews 余额不足',
