 ...
 *   429/408/5xx 可重试；400 读体嗅探区分参数错误与恒可重试语义；
 *   其余非 2xx 默认不可重试。
...
async function fetchSingle(target, apiKey, endpoint, zone, options, fetchImpl) {
  ...
  // 400 读体嗅探：命中恒可重试语义（timeout/bot_blocked/target_unreachable）则可重试，否则视为参数错误不可重试。
  if (res.status === 400) {
    let bodyText = '';
    try {
      if (res && typeof res.text === 'function') bodyText = String((await res.text()) ?? '');
    } catch {
      bodyText = '';
    }
    const sniffed = innerCode(bodyText);
    if (sniffed !== 'UPSTREAM_ERROR') {
      throw fail(sniffed, 'BrightData 抓取失败：' + String(target) + ' ' + (bodyText || 'HTTP 400').slice(0, 500), {
        status: 400,
        retryable: true,
      });
    }
    throw fail('INVALID_REQUEST', 'BrightData 非法请求（400）：' + String(target) + (bodyText ? ' ' + bodyText.slice(0, 500) : ''), {
      status: 400,
      retryable: false,
    });
  }
